...
| `npm run test:merchant-api` avec SQLite, puis `TEST_MERCHANT_POSTGRES_URL=postgresql://postgres@127.0.0.1:55473/merchant_validation_api3 npm run test:merchant-api` | ✅ Réussi dans **les deux moteurs**. | Isolation inter-marchands ; consentement et révocation ; téléphone E.164 ; montant/devise ; lien 48 h avec jeton haché ; déclaration puis confirmation authentifiée et journalisée ; invitation à usage unique ; limites de tentatives ; QR en **stockage local SQLite seulement**. |
...
- `https://kouturepro.vercel.app/api/health` : HTTP 200, `ok=true`, `database=ready` **pour l’application déjà déployée**.
