...
Ne testez pas une Preview en la reliant à Neon **Production** : la Function applique les nouvelles migrations additives au premier démarrage et les tests créent des comptes/commandes. Pour la Preview, une **branche de base Neon temporaire** avec ses variables Preview est préférable ; elle ne remplace pas la base Neon Production existante. Le code utilise l’URL Vercel propre à la Preview pour l’authentification et les liens, même si `PUBLIC_BASE_URL` définit le domaine Production.
